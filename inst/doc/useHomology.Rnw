...
% \VignetteDepends{hsahomology, xlahomology}
...
similarity (homoType. B - reciprocal best match between three or more
...
the obtained HGIDs to locate homologous genes in other organisms using
...
containing homologous genes of best matches with percent
...
is \Robject{xlahomologyHGID} in \Rpackage{xlahomology}, which is a
