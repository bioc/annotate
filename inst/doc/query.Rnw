...
% \VignetteDepends{Biobase, annotate, XML, edd, class, genClass, mva}
...
\begin{document}
\title{Overview: Automated Querying of PubMed Data}
\maketitle{}
...
<<>>=
hgu95pm <- read.annotation("hgu95Apmed")
ids <- multiget(affys,envir=hgu95pm)
...
This URL can then be used (either using the \verb+openBrowser+
...
b <- xmlRoot(x)
@
At this point of time, the object 'b' can be manipulated in a similar
manner as 'a' from the PubMed example.
...
specifying \{em disp="browser"} in the parameter listing.
