...
\item[locuslinkByID] Users specify Entrez Gene identifiers and the
...
The function \Rfunction{locuslinkByID} takes a set of known Entrez Gene
...
from our sample data.  However, \verb+sample.exprSet.1+ provided us with
