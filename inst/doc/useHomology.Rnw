...
\begin{document}
\title{How to use the \Rpackage{homology} packages}
...
The \Rpackage{homology} packages was built based on a source
data file provided by HomoloGene
(\url{http://www.ncbi.nlm.nih.gov/HomoloGene/}).
This vignette illustrates how to use the \Rpackage{homology} packages
...
packages by performing two tasks to introduce users to the
packages, although the potential use of the packages is well beyond the
scope of the tasks.

\section*{Contents of the package}

The \Rpackage{homology} packages are built on organism
bases with a separate data package for each of the species contained
in the source file. The packages are named following a convention of
\Rpackage{XXXhomology} where XXX are a three letter string consisting of the
first letter of the genus name and the first two letters of the
species name of a given organism. For example, hsahomology is the
package for human (Homo sapiens).

As other Bioconductor data packages, the \Rpackage{homology} packages
...
environment objects is shown below (using the package for human as an
example:
...
for (psid.Dr in psids.Dr) {
  ...
  for (homoGeneList in listOfLists) {
    for (homoGene in homoGeneList) {
      if (homoGene[["homoOrg"]] == 'Xenopus laevis') {
  ...
  if (!is.null(targetHomoGene)) {
    ...
    acc <- sub('\\.[0-9]+$', '', targetHomoGene[["homoACC"]])
