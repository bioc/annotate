...
igenes <- geneNames(sample.exprSet.1)[246:260]
@
...
<<expDat>>=
dat <- exprs(sample.exprSet.1)[igenes,1:10]
FC <- rowMeans(dat[igenes,1:5]) - rowMeans(dat[igenes,6:10])
pval <- esApply(sample.exprSet.1[igenes,1:10], 1, function(x) t.test(x[1:5], x[6:10])$p.value)
tstat <- esApply(sample.exprSet.1[igenes,1:10], 1, function(x) t.test(x[1:5], x[6:10])$statistic)
@
