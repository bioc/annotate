...
environment objects is shown below (based on version 1.6.0 of
\Rpackage{homology}):
...
\Robject{homology9606} contains data for homologous gene found in other
organisms having homologous relationships with human genes. Organism codes
and the corresponding scientific names of organisms can be accessed
using the \Robject{homologyORGCODE} and are shown below:

<<>>=
cbind(as.vector(homologyORGCODE), names(homologyORGCODE))
@

Environments with an organism code component in their names have
HomologGeneIDs as keys and lists of homoData objects for other
organisms that have been identified to have homology relationships as
the corresponding values. The list for a given HomologGeneID may have
one to many homoData objects depending on whether homolgy
...
LocusLink id (homoLL), HomologGeneID (homoHGID), GenBank accession
number (homoACC) of the best matching sequence, type of similarity
...
LocusLink identifiers and HGIDs (HomoloGeneIDs used by HomoloGene to
...
need by finding out the organism code for human.

<<>> =
homologyORGCODE["Homo sapiens"]
@

Now we can get the data for homologous genes in other organisms for
LocusLink id 13797 using the correct environment.

<<>>=
homoGenes <- homology9606[[as.character(homologyLL2HGID[["13797"]])]]
@

\Robject{homoGenes} is a list of homoData objects. We are only
...
geneList <- list()
for(i in homoGenes){
    if(homoType(i) == "B" && homoPS(i) > 80.00){
        geneList[[length(geneList) + 1]] <- i
    }
}
geneList
@
...
<<>>=
sapply(geneList, function(x) return(homoOrg(x)))
sapply(geneList, function(x) return(homoLL(x)))
sapply(geneList, function(x) return(homoHGID(x)))
@

\subsubsection{Task 2}

Find all the genes in \texttt{Xenopus laevis} that are homologous to
genes of \texttt{Danio rerio} with percent indetity (homoType = B or
...
Gens in \texttt{Xenopus laevis} that satisfy the conditions are:
