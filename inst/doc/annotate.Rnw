...
meta-data. One is to used highly curated data that have been assembled
...
resources are provided in \cite{PubMedRnews; PubMedVignette}.
...
(\url{http://www.bioconductor.org/data/meta-data} that are available
...
\Rfunction{getLL}, \Rfunction{getGO} and so on. There are two reasons
